...
- 20 eşya: gerekli ekipmanlar, bakım seti, tüketilebilirler, araç ve ev; kullanım, değer kaybı, satış ve bakım.
...
3.3 güncellemesi eski puanları, para ve ilişkileri geri almaz. Eski karizma puanı güzellik olarak korunur; geçmişe ait doğum puanı uydurulmaz. Ebeveyn özellikleri aynı kayıt için tutarlı şekilde tamamlanır ve rastgele olayların akışı değiştirilmez. Gerçek aileden gelen başlangıcı doğumdan deneyimlemek için mevcut hayatını arşivleyip yeni hayat başlatabilirsin. Karakterin adına dokunarak aile karşılaştırmasını aç; avatarına dokunarak saçını düzenle.

Favoriler ve isteğe bağlı **yaş atlarken kalan zamanı hatırlat** tercihi cihazda ayrı tutulur. Hatırlatma varsayılan olarak kapalıdır. Günlük giriş zorunluluğu, gerçek zamanlı bekleme duvarı ve ücretli rastgele ödül yoktur. Tasarımın gerekçeleri: [UX.md](UX.md). Ölçülmüş gelişim ve denge sınırları: [BALANCE.md](BALANCE.md). Özellikle ileri kariyer ekonomisi için oyun testleri sürecektir.
...
Testler olay ve veri tutarlılığını, kayıt devamlılığını, ekipman/tüketim sınırlarını, tam yıllık bütçeyi, eğitim ve sağlık yollarını, sosyal etkileşim kısıtlarını ve 24 tam yaşamı doğrular. Genetik testleri iki ebeveynin etkisini, dağılımı, kardeş/çocuk mirasını ve eski kayıt göçünü denetler. Arayüz testleri kalıcı barları, olay/rehber pencerelerini, filtre/favori/önizlemelerin kaynak tüketmemesini, isteğe bağlı yaş onayını ve kayıt içe aktarmayı kontrol eder. Görsel yerleşim ayrıca gerçek tarayıcıda sınanır.
...
Tipografi cihazın yerleşik yazı tiplerini kullanır; harici font isteği yoktur. Eski raster prototip görseli `assets/` içinde kaynak olarak saklanmıştır; yeni sürüm karakterleri kodla çizer. Genetik, güzellik, sağlık ve ekonomi puanları kurgu oyun kurallarıdır; bilimsel veya kişisel değer ölçümleri değildir.
