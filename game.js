...
(() => {
  'use strict';
  const E = globalThis.LifeEngine, D = {...globalThis.LifeData,actions:E?.actions||globalThis.LifeData?.actions}, A = globalThis.LifeAvatar;
  ...
  let activeActivity='',scrollPositions={},renderRevision=0;
  ...
  function dispatch(type, payload = {}) {
    try {
      ...
      if (type === 'choice' || type==='ackNotice' || (type==='activity'&&$('#dialog').open&&$('#dialog').dataset.kind==='activity-confirm')) closeDialog(false);
      save(); render();
  ...
  function heading(title,description='') {return '<header class="page-heading"><div><h1>'+title+'</h1>'+(description?'<p>'+description+'</p>':'')+'</div></header>';}
  ...
  function inspectActivity(id) {
    ...
    dialog(a.name,'<div class="activity-detail-hero"><span class="activity-symbol">'+esc(a.icon||'✦')+'</span><p>'+esc(a.description)+'</p></div><div class="activity-cost-row"><span>'+icon('energy')+(a.energy||1)+' zaman</span><span>'+icon('wallet')+(cost?money(cost):'Ücretsiz')+'</span></div><h3>Kazanım önizlemesi</h3><div class="effect-chips">'+effectChips+xp+(preview.effects.money?'<span class="effect-chip '+(preview.effects.money>0?'positive':'negative')+'">'+moneyPill(preview.effects.money)+'</span>':'')+'</div>'+(preview.repeat?'<p class="helper-text">Aynı etkinliği bu yıl tekrarlıyorsun; kazanımı azalıyor.</p>':'')+'<p class="helper-text">Kazanımlar mevcut durumuna göre hesaplanır. Karşılaşmalar ve özel sonuçlar ayrıca etkileyebilir.</p>'+(why?'<div class="activity-help">'+icon('lock')+'<span>'+esc(why)+'</span></div>':''),{kind:'activity-info',activityId:id});
  }
  function requestActivity(id,key,button) {
    if(!state||$('#dialog').open||key!==String(renderRevision))return;
    const a=D.actions.find(a=>a.id===id);if(!a)return;
    const why=E.actionReason(state,a);if(why){toast(why);return;}
    const cost=E.costOf(state,a);
    // Confirm material purchases, not everyday actions. Thresholds follow the
    // same price index as the displayed cost; relative checks use available cash.
    const significant=cost>=price(5000)||(cost>=price(1000)&&cost>=state.money*.25);
    if(significant){
      dialog('Bu harcamayı yapalım mı?','<p><strong>'+esc(a.name)+'</strong> için '+(a.energy||1)+' zaman kullanacaksın.</p><div class="spending-summary"><div class="spending-line"><span>Ücret</span><strong class="confirm-cost">'+money(cost)+'</strong></div><div class="spending-line"><span>Birikimin</span><b>'+money(state.money)+'</b></div><div class="spending-line"><span>İşlem sonrası</span><b>'+money(state.money-cost)+'</b></div></div><p class="helper-text">Bu tutar yüksek olduğu veya birikiminin önemli bir bölümünü kullandığı için onayını istiyoruz.</p><div class="row wrap"><button class="button" data-do="closeDialog">Vazgeç</button><button class="button primary" data-do="confirmActivity" data-id="'+id+'" data-action-key="'+key+'">'+money(cost)+' öde ve yap</button></div>',{kind:'activity-confirm',activityId:id,actionKey:key});
      return;
    }
    button.disabled=true;if(!dispatch('activity',{id}).ok)button.disabled=false;
  }
  function journal() {
  ...
  function activities() {
    return heading('Aktiviteler')+
      ...
      '<div class="activity-toolbar"><div class="search-controls"><label class="search-field">'+icon('search')+'<input type="search" id="activitySearch" aria-label="Aktivite ara" placeholder="Aktivite ara…" value="'+esc(activityQuery)+'" maxlength="80" autocomplete="off"></label><select id="activityCategory" aria-label="Aktivite kategorisi">'+Object.entries(categories).map(([k,v])=>'<option value="'+k+'" '+(category===k?'selected':'')+'>'+v+'</option>').join('')+'</select></div><div class="filter-buttons" aria-label="Aktivite durumu">'+[['age','Bu yaşta'],['ready','Yapılabilir'],['favorites','Favoriler']].map(([k,v])=>'<button class="'+(activityFilter===k?'active':'')+'" aria-pressed="'+(activityFilter===k)+'" data-activity-filter="'+k+'">'+v+'</button>').join('')+'</div></div><div id="activityResults">'+activityResults()+'</div>');
  }
  function activityResults() {
    const query=activityQuery.toLocaleLowerCase('tr-TR').trim();
    const currentPathActions=new Set(E.paths(state).flatMap(p=>p.actionIds||[p.actionId]));
    const pool=D.actions.filter(a=>(!a.pathId||showFuture||activityFilter==='favorites'||currentPathActions.has(a.id))&&(category==='all'||a.category===category)&&(!query||(a.name+' '+a.description).toLocaleLowerCase('tr-TR').includes(query)));
  ...
  function future() {
    ...
    let body='';
    if(futureView==='paths')body=pathPanel();
    else if(futureView==='education')body='<div class="guide-tip">'+icon('book')+'<span>Eğitim yılda 2 zaman ayırır; kayıt ayrıca 1 zaman ister. Programları not, uzmanlık ve burs sonrası bütçenle birlikte değerlendir.</span></div><div class="detail-list">'+D.courses.map(program).join('')+'</div>';
    ...
    if(futureView==='overview')body=circlePanel()+body+'<button class="next-step" data-future-view="paths"><span class="next-step-icon">'+icon('compass')+'</span><span><b>Hayat yollarını keşfet</b><small>Araştırma, spor ve müzik · Yıllara yayılan hedefler</small></span>'+icon('chevron')+'</button>';
    return heading('Gelecek')+'<div class="tabs future-tabs" aria-label="Gelecek görünümü">'+[['overview','Şu an'],['paths','Hayat yolları'],['education','Eğitimler'],['jobs','İş ilanları']].map(([k,v])=>'<button class="tab '+(futureView===k?'active':'')+'" aria-pressed="'+(futureView===k)+'" data-future-view="'+k+'">'+v+'</button>').join('')+'</div>'+body;
  }
  function pinnedPath() {
    const p=E.paths(state).find(p=>p.id===state.lifePaths?.pinned);
    return p?'<button class="next-step pinned-path" data-future-view="paths"><span class="next-step-icon">'+icon('pin')+'</span><span><b>'+esc(p.name)+' · '+esc(p.stageLabel)+'</b><small>'+esc(p.goal)+'</small></span>'+icon('chevron')+'</button>':'';
  }
  function pathPanel() {
    return '<p class="helper-text path-intro">Yollar aynı zaman bütçesini paylaşır. Bir hedefi sabitle; adımlarını yıllara yay.</p><div class="path-list">'+E.paths(state).map(p=>{
      const pinned=state.lifePaths?.pinned===p.id;
      const ids=p.actionIds||[p.actionId].filter(Boolean);
      const actions=ids.map(id=>D.actions.find(a=>a.id===id)).filter(Boolean).sort((a,b)=>Number(b.id===p.actionId)-Number(a.id===p.actionId));
      const npc=state.npcs.find(n=>n.id===p.npcId);
      const nextAgeText=p.nextAge+' yaşında '+(p.stage==='preparation'?'hazırlığa devam edebilirsin.':p.stage==='development'?'sonraki adım açılır.':'yeni bir gelişme bekleniyor.');
      return '<article class="card path-card" data-path="'+esc(p.id)+'"><div class="path-heading"><div><p class="eyebrow">'+esc(p.stageLabel)+'</p><h2>'+esc(p.icon)+' '+esc(p.name)+'</h2></div><button class="icon-button '+(pinned?'is-pinned':'')+'" data-do="pinPath" data-id="'+esc(p.id)+'" aria-pressed="'+pinned+'" aria-label="'+esc(p.name)+': '+(pinned?'sabitlemeyi kaldır':'hedefi sabitle')+'">'+icon('pin')+'</button></div><p>'+esc(p.description)+'</p>'+(p.started?'<div class="path-goal"><b>'+(p.completed?'Bu yoldaki izin':'Sıradaki adım')+'</b><p>'+esc(p.goal)+'</p>'+(p.nextAge>state.age?'<small>'+esc(nextAgeText)+'</small>':'')+'</div>':'')+(npc?'<button class="text-link" data-do="person" data-id="'+esc(npc.id)+'">'+esc(npc.name)+(npc.alive?' · Yol arkadaşın':' · Hatıralarında')+' →</button>':'')+actions.map(activityCard).join('')+(p.outcome==='professional'?'<button class="text-link" data-future-view="jobs">Açılan meslekleri incele →</button>':'')+(p.history?.length?'<details class="path-history"><summary>Bu yolun hikâyesi · '+p.history.length+' an</summary>'+p.history.slice(-8).reverse().map(h=>'<p><small>'+h.age+' yaş</small> '+esc(h.text)+'</p>').join('')+'</details>':'')+'</article>';
    }).join('')+'</div>';
  }
  function circlePanel() {
    const c=E.circle(state);if(!c.available)return '';
    const action=D.actions.find(a=>a.id===c.actionId);
    return '<section class="card circle-card"><h2>'+esc(c.title)+'</h2><p class="helper-text">Aynı insanlar, yeni yıllar. Tanışmak arkadaş olmak demek değil; yakınlığı sen şekillendir.</p>'+(c.members.length?'<div class="circle-members">'+c.members.map(n=>'<button class="button small" data-do="person" data-id="'+esc(n.id)+'">'+esc(n.name)+' · '+esc(roleNames[n.role]||n.role)+'</button>').join('')+'</div>':'')+(action?activityCard(action):'')+'</section>';
  }
  function assets() {
  ...
  function dialog(title,body,options={}) {
    ...
    el.dataset.kind=options.kind||'general';el.dataset.title=title;el.dataset.activityId=options.activityId||'';el.dataset.actionKey=options.actionKey||'';
...
'<div class="divider"></div><h3>Senin oyun ritmin</h3><button class="event-choice preference-toggle" data-do="toggleAgeConfirm" role="switch" aria-checked="'+preferences.confirmAge+'"><span class="grow"><b>Kalan zamanı hatırlat</b><small>Yaş atlarken kullanmadığın zaman için onay sor. Varsayılan olarak kapalı.</small></span><span class="toggle-state">'+(preferences.confirmAge?'Açık':'Kapalı')+'</span></button><p class="helper-text">Favorilerin bu tarayıcıda saklanır. Bildirim, günlük giriş zorunluluğu veya gerçek zamanlı bekleme yok.</p><div class="divider"></div><p class="helper-text">BİR ÖMÜR / Her Hayat Başka · 4.0<br>Ekonomi, sağlık ve genetik oyun için tasarlanmış kurmaca kurallardır.</p>');
  ...
  function person(id,feedback='') {
    ...
    dialog(p.name,'<div class="dialog-preview"><span class="npc-avatar">'+avatar(p)+'</span><div><h3>'+esc(roleNames[p.role]||p.role)+'</h3><p>'+p.age+' yaş · '+esc(p.personality||'')+'<br>'+esc(p.job||'')+'</p><div class="row wrap" style="margin-top:8px"><span class="pill">Yakınlık '+num(p.bond)+'</span><span class="pill">Güven '+num(memory.trust)+'</span></div></div></div>'+(p.traits?'<p class="helper-text">Zekâ '+point(p.traits.intelligence)+' · Güzellik '+point(p.traits.beauty)+'</p>':'')+'<p class="helper-text">'+esc(memory.label)+' · '+esc(memory.explanation)+'</p>'+(memory.promiseText?'<div class="guide-tip">'+icon('clock')+'<span>'+esc(memory.promiseText)+'</span></div>':'')+
      (memory.interest?'<div class="guide-tip">'+icon('people')+'<span><b>İlgi alanı: '+esc(memory.interest.name)+'</b><br>'+esc(memory.responseHint||'')+'</span></div>':'')+
  ...
  document.addEventListener('change',e=>{
    if(e.target.id==='activityCategory'){if(Object.hasOwn(categories,e.target.value)){category=e.target.value;render();$('#activityCategory')?.focus({preventScroll:true});}return;}
  ...
  document.addEventListener('click',e=>{
    ...
    if(action==='pinPath')return dispatch('pinPath',{id:state.lifePaths?.pinned===id?null:id});
    ...
    if(action==='activity')return requestActivity(id,b.dataset.actionKey,b);
    if(action==='confirmActivity'){
      const el=$('#dialog');if(!el.open||el.dataset.kind!=='activity-confirm'||el.dataset.activityId!==id||el.dataset.actionKey!==b.dataset.actionKey||b.dataset.actionKey!==String(renderRevision))return;
      b.disabled=true;const response=dispatch('activity',{id});if(!response.ok)b.disabled=false;return;
    }
    if(['buy','sell','use','enroll','apply','quit','retire'].includes(action)){b.disabled=true;const response=dispatch(action,{id});if(!response.ok)b.disabled=false;if(response.ok&&['quit','retire'].includes(action)&&!state.notices.length&&!state.pending)closeDialog();}
  });
