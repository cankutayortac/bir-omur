...
(function (root) {
  'use strict';
  const actions = [
    { id: 'play', name: 'Hayal oyunları', description: 'Oyuncaklarınla bir dünya kur. Küçük yaşta merak ve güven kazan.', icon: '🧸', category: 'creative', minAge: 0, maxAge: 8, energy: 1, perYear: 2, effects: { happiness: 7, knowledge: 2, charisma: 1 } },
    ...
    { id: 'study_group', name: 'Çalışma grubu', description: 'Soruları birlikte çöz; sınıfından yeni insanlarla tanışabilirsin.', icon: '📝', category: 'learning', minAge: 10, maxAge: 27, energy: 1, perYear: 2, requires: { school: true }, effects: { grade: 6, knowledge: 3, charisma: 2 }, meetChance: 0.42 },
    { id: 'code', name: 'Kişisel yazılım projesi', description: 'Bilgisayarınla küçük bir uygulama geliştir. Bilgi ve iş performansı kazandırır.', icon: '💻', category: 'learning', minAge: 12, energy: 2, perYear: 2, requires: { items: ['laptop'], stats: { knowledge: 25 } }, effects: { knowledge: 9, performance: 4, stress: 3, flag: 'portfolio' } },
    { id: 'language', name: 'Dil pratiği', description: 'Ücretsiz konuşma grubunda kendini ifade etmeyi öğren.', icon: '🌍', category: 'learning', minAge: 12, energy: 1, perYear: 2, effects: { knowledge: 3, charisma: 4 }, meetChance: 0.3 },
    { id: 'debate', name: 'Münazara kulübü', description: 'İddianı kaynaklarla savun; karizma ve bilgi birlikte gelişir.', icon: '🎙️', category: 'social', minAge: 13, maxAge: 30, energy: 1, perYear: 2, requires: { stats: { knowledge: 25 } }, effects: { charisma: 6, knowledge: 3, happiness: 2 }, meetChance: 0.36 },
    { id: 'socialize', name: 'Mahallede sosyalleş', description: 'Bir yürüyüş, kısa bir sohbet… Yeni bir arkadaşlığın başlangıcı olabilir.', icon: '👋', category: 'social', minAge: 5, energy: 1, perYear: 2, effects: { charisma: 4, happiness: 5, stress: -3 }, meetChance: 0.55 },
    { id: 'cafe', name: 'Kafede buluş', description: 'Arkadaş çevreni genişlet. Harcaman tek bir buluşmanın ücretidir.', icon: '☕', category: 'social', minAge: 16, energy: 1, cost: 650, perYear: 2, effects: { charisma: 5, happiness: 6, stress: -4 }, meetChance: 0.5 },
    { id: 'volunteer', name: 'Gönüllü çalış', description: 'Mahalle dayanışmasına katıl; birlikte çalıştığın insanlarla bağ kur.', icon: '🤲', category: 'social', minAge: 12, energy: 1, perYear: 2, effects: { happiness: 5, charisma: 3, knowledge: 2, flag: 'volunteer' }, meetChance: 0.45 },
    { id: 'community', name: 'Topluluk etkinliği', description: 'Yerel bir etkinliğe katıl. Yetişkinlikte yeni dostluklara alan aç.', icon: '🎟️', category: 'social', minAge: 18, energy: 1, cost: 400, perYear: 2, effects: { charisma: 5, happiness: 4 }, meetChance: 0.6 },
    ...
    { id: 'photography', name: 'Fotoğraf yürüyüşü', description: 'Fotoğraf makinenle hikâyeler yakala ve meraklılarla tanış.', icon: '📷', category: 'creative', minAge: 14, energy: 1, perYear: 2, requires: { items: ['camera'] }, effects: { charisma: 4, happiness: 6, knowledge: 2 }, meetChance: 0.35 },
    { id: 'perform', name: 'Açık mikrofon gecesi', description: 'Gitarınla sahneye çık. Küçük bir ücret ve bolca özgüven kazan.', icon: '🎤', category: 'creative', minAge: 16, energy: 2, perYear: 1, requires: { items: ['guitar'], stats: { charisma: 35 } }, effects: { money: 1600, charisma: 7, happiness: 5, stress: 3 }, meetChance: 0.48 },
    ...
    { id: 'mentor', name: 'Deneyimini paylaş', description: 'Gençlere ücretsiz yol göster; tecrüben başka birinin hayatına dokunsun.', icon: '🧑‍🏫', category: 'social', minAge: 45, energy: 1, perYear: 2, requires: { stats: { knowledge: 45 } }, effects: { charisma: 4, happiness: 7, stress: -4 }, meetChance: 0.4 },
  ...
  const careers = [
    ...
    { id: 'barista', name: 'Barista', icon: '☕', description: 'Müşteri ilişkileri ve hızlı çalışma.', salary: 264000, minAge: 18, requires: { stats: { charisma: 24 } } },
    { id: 'receptionist', name: 'Resepsiyon görevlisi', icon: '🛎️', description: 'İletişim gücünü düzenli bir işe dönüştür.', salary: 288000, minAge: 18, requires: { stats: { charisma: 30, knowledge: 20 } } },
    { id: 'sales', name: 'Satış temsilcisi', icon: '🤝', description: 'Hedef baskısı yüksek, ilerleme alanı geniş.', salary: 336000, minAge: 18, requires: { stats: { charisma: 42, knowledge: 22 } } },
    ...
    { id: 'trainer', name: 'Spor eğitmeni', icon: '🏅', description: 'Kondisyonunu ve iletişimini mesleğe dönüştür.', salary: 372000, minAge: 18, requires: { degree: 'fitness', stats: { strength: 55, health: 60, charisma: 25 } } },
    { id: 'designer', name: 'Görsel tasarımcı', icon: '🎨', description: 'Tasarım eğitimi ve bilgisayar gerekir.', salary: 432000, minAge: 18, requires: { degree: 'design', items: ['laptop'], stats: { knowledge: 40, charisma: 30 } } },
    ...
    { id: 'teacher', name: 'Öğretmen', icon: '🧑‍🏫', description: 'Alan bilgisi kadar insanlarla bağ kurmayı da gerektirir.', salary: 480000, minAge: 21, requires: { degree: 'education', stats: { knowledge: 52, charisma: 35 } } },
    ...
    { id: 'engineer', name: 'İnşaat mühendisi', icon: '🏗️', description: 'Planlama, teknik bilgi ve saha sorumluluğu.', salary: 564000, minAge: 21, requires: { degree: 'engineering', stats: { knowledge: 58, charisma: 25 } } },
    { id: 'lawyer', name: 'Avukat', icon: '⚖️', description: 'Hukuk eğitimi, güçlü analiz ve ikna kabiliyeti gerekir.', salary: 660000, minAge: 22, requires: { degree: 'law', stats: { knowledge: 62, charisma: 45 } } },
  ...
  const courses = [
    ...
    { id: 'education', name: 'Eğitim fakültesi', icon: '📚', description: 'Dört yıl boyunca alan bilgisi ve öğretmenlik becerileri.', degree: 'education', duration: 4, annualCost: 16000, minAge: 18, requires: { stats: { knowledge: 40, charisma: 25 } } },
  ...
  const events = [
    event('first_steps', 'İlk küçük keşif', 'Sehpanın kenarına tutunup ayağa kalktın. Birkaç adım ötede en sevdiğin oyuncak duruyor.', '🧸', 1, 2, [
      ...
      choice('Bir el uzatılmasını bekle', 'Ailenin elini tuttun; birlikte denemek sana güven verdi.', { bond: 4, happiness: 3, charisma: 1 })
    ], { once: true }),
    ...
    event('playground_share', 'Kumdan kale', 'Parkta başka bir çocuk kalenin yanına kendi tünelini yapmak istiyor.', '🏖️', 3, 6, [
      choice('Birlikte büyük bir kale yap', 'İki küçük kale bir şehre dönüştü. Yeni bir oyun arkadaşı edindin.', { charisma: 3, happiness: 4, meet: 'friend' }),
    ...
    event('library_card', 'İlk kütüphane kartın', 'Çocuk kütüphanesinde görevli, resimli kitapları eve götürebileceğini söyledi.', '📚', 4, 7, [
      ...
      choice('Hikâye saatine katıl', 'Masal dinlerken yanındaki çocukla sohbete başladın.', { charisma: 3, happiness: 3, meet: 'friend' })
    ], { once: true }),
    event('broken_vase', 'Kırılan vazo', 'Salonda top oynarken vazoyu devirdin. Sesleri duyan ailen odaya geliyor.', '🏺', 4, 8, [
      choice('Olanı dürüstçe anlat', 'Üzüldüler ama dürüstlüğünü takdir ettiler. Birlikte topladınız.', { bond: 4, charisma: 2, stress: -2 }),
    ...
    event('first_school', 'Sınıfın kapısında', 'İlk okul gününde herkes birbirine yabancı. Öğretmenin boş bir sırayı gösteriyor.', '🎒', 6, 7, [
      choice('Yanındaki çocuğa adını söyle', 'Kısa sohbet teneffüse kadar sürdü. İlk okul arkadaşını buldun.', { charisma: 4, happiness: 3, meet: 'friend' }),
    ...
    event('forgotten_homework', 'Defter evde kaldı', 'Ödevini yaptın ama defterini masada unuttun. Öğretmenin kontrol etmeye başladı.', '📓', 7, 12, [
      choice('Durumu açıkla, yarın getir', 'Sözünü tuttun. Küçük bir not kaybıyla konuyu kapattın.', { grade: -2, charisma: 2 }),
      ...
      choice('Konuyu tahtada çöz', 'Bilginle hazırlıklı olduğunu gösterdin.', { grade: 5, charisma: 2 }, { requires: { stats: { knowledge: 30 } } })
    ]),
    event('school_bully', 'Teneffüste kırıcı sözler', 'Bir öğrenci arkadaşlarının önünde seninle alay ediyor. Diğerleri ne yapacağını bekliyor.', '🎒', 7, 16, [
      choice('Güvendiğin bir öğretmene anlat', 'Öğretmenin konuyu takip etti. Tek başına olmadığını hissettin.', { happiness: 3, stress: -4, charisma: 1 }),
      choice('Sakin ve net sınır koy', 'Sesini yükseltmeden kendini savundun.', { charisma: 5, happiness: 4 }, { requires: { stats: { charisma: 28 } } }),
    ...
    event('team_selection', 'Takım seçmeleri', 'Beden eğitimi öğretmenin seni okul takımının denemesine çağırdı. Seçmeler haftaya.', '⚽', 8, 16, [
      ...
      choice('Maç organizasyonuna yardım et', 'Takımın yanında başka bir rol buldun; öğretmenin sorumluluğunu fark etti.', { charisma: 3, grade: 2, happiness: 2 }),
    ...
    event('school_friend_moves', 'Boş kalacak sıra', 'Yakın arkadaşın ailesiyle başka bir şehre taşınacağını söyledi. Sıradaki sessizlik şimdiden tuhaf geliyor.', '📮', 9, 17, [
      choice('İletişimde kalmak için sözleş', 'Ayrılık üzdü ama arkadaşlığı sürdürmeye karar verdiniz.', { bond: 5, happiness: -2, charisma: 2 }),
    ...
    event('exam_pressure', 'Sınav haftası', 'Üst üste sınavların var. Takvimi görünce hangi dersten başlayacağını şaşırdın.', '📝', 11, 27, [
      ...
      choice('Bir dersten yardım iste', 'Anlamadığın yeri kabul etmek işini kolaylaştırdı.', { grade: 4, charisma: 2, stress: -2, meet: 'friend' })
    ], { requires: { school: true } }),
    event('group_project', 'Grup ödevinde yük paylaşımı', 'Grup ödevinin teslimi yaklaşırken herkes senden daha fazlasını bekliyor.', '🗂️', 12, 24, [
      choice('İşleri adil biçimde bölüştür', 'Beklentileri açıkça konuştunuz; herkes bir bölüm üstlendi.', { grade: 5, charisma: 4, stress: -2 }),
    ...
    event('social_media', 'Paylaş tuşunun önünde', 'Bir arkadaşının komik ama mahcup edici fotoğrafı grubunda dolaşıyor. Senden de paylaşmanı istediler.', '📱', 13, 23, [
      choice('Arkadaşının iznini sor', 'Fotoğrafın paylaşılmasını istemediğini öğrendin. Sana güvendi.', { charisma: 3, happiness: 3, bond: 4 }),
      choice('Gruptan sessizce çık', 'Tartışmaya girmeden sınırını korudun.', { stress: -3 }),
      choice('Beğeni toplamak için paylaş', 'Kısa süreli ilginin ardından arkadaşın kırıldı.', { charisma: 1, bond: -8, stress: 5, happiness: -3 })
    ...
    event('audition_seed', 'Gençlik sahnesi', 'Mahalle kültür merkezi genç müzisyenleri arıyor. Sahneye çıkma fikri hem heyecanlandırıyor hem korkutuyor.', '🎸', 13, 22, [
      choice('Seçmelere hazırlan', 'Bir repertuvar hazırlamaya başladın. Gelecek yıl çağrılacaksın.', { charisma: 3, stress: 3, flag: 'audition_ready' }, { requires: { items: ['guitar'] }, schedule: { id: 'audition_final', after: 1 } }),
      choice('Sahne arkasına gönüllü ol', 'Organizasyonun parçası oldun; kuliste yeni insanlarla tanıştın.', { charisma: 4, happiness: 3, meet: 'friend' }),
    ...
    event('first_crush', 'Konuşmak için bir bahane', 'Etkinlikte hoşuna giden biri var. İkiniz de aynı kitabın standında duruyorsunuz.', '💌', 16, 29, [
      choice('Kitap hakkında sohbet aç', 'Ortak bir konudan konuşmak kolay geldi. Yeni biriyle tanıştın.', { charisma: 3, happiness: 4, meet: 'friend' }),
      choice('Duygularını hemen anlat', 'Dürüstçe bir adım attın.', {}, { chance: { stat: 'charisma', target: 36, success: { effects: { happiness: 7, charisma: 3, meet: 'friend' }, text: 'Gülümseyip sohbeti sürdürdü. Birlikte vakit geçirmek için sözleştiniz.' }, failure: { effects: { happiness: -3, charisma: 2 }, text: 'Aynı ilgiyi paylaşmıyordu. Saygıyla kabul ettin ve deneyim kazandın.' } } }),
    ...
    event('teen_party', 'Bir ev partisi', 'Arkadaşların hafta sonu bir parti düzenliyor. Ertesi gün tamamlaman gereken işler de var.', '🎉', 15, 24, [
      choice('Kısa süre uğra', 'Sohbet ettin, zamanında döndün. Yeni biriyle tanıştın.', { happiness: 5, charisma: 3, meet: 'friend' }, { cost: 300 }),
      choice('Geceyi uzat', 'Eğlendin ama ertesi günü yorgun geçirdin.', { happiness: 8, charisma: 4, health: -4, grade: -3, stress: 3 }, { cost: 800 }),
    ...
    event('career_day', 'Meslek günündeki stant', 'Okulun meslek gününde farklı alanlardan insanlar var. Her standın anlattığı hayat başka.', '🧭', 15, 18, [
      ...
      choice('Bir konuşmacıya soru sor', 'Mesleğin zor yanlarını da dinledin. Beklentilerin netleşti.', { charisma: 3, knowledge: 3 }),
    ...
    event('university_orientation', 'Kampüste ilk hafta', 'Kayıt işlemleri bitti. Kulüpler tanıtım yapıyor, üst sınıflar tavsiye dağıtıyor.', '🎓', 18, 30, [
      ...
      choice('Kampüsü bir grupla keşfet', 'Kaybolarak da olsa yerleri öğrendiniz. Bir arkadaşlık başladı.', { charisma: 4, happiness: 4, meet: 'friend' }),
    ...
    event('roommate_conflict', 'Ortak alanda anlaşmazlık', 'Birlikte kaldığın kişi temizlik ve masraf paylaşımına uymuyor. Birikmiş bir gerginlik var.', '🏠', 18, 35, [
      choice('Yazılı bir paylaşım planı öner', 'Beklentiler açık hâle gelince konuşmak kolaylaştı.', { charisma: 4, stress: -6 }),
    ...
    event('work_credit', 'Fikrinin altında başka bir isim', 'Toplantıda önerdiğin fikir, raporda bir başkasına yazılmış. Yöneticin henüz durumu bilmiyor.', '📋', 18, 65, [
      choice('Belgelerinle sakin bir görüşme yap', 'Katkını açıkça gösterdin. Yöneticin düzeltme yaptı.', { performance: 6, charisma: 3, stress: -2 }),
      choice('Herkesin önünde suçla', 'Konu görünür oldu ama ekip içindeki gerginlik arttı.', { charisma: 1, performance: -4, stress: 7 }),
    ...
    event('overtime_request', 'Acil teslim, uzun akşam', 'Yöneticin son anda yetişmesi gereken bir işi akşam da sürdürmeni istiyor.', '🌃', 18, 65, [
      ...
      choice('Süre ve iş kapsamını konuş', 'İşi daha gerçekçi bir takvime yerleştirdiniz.', { performance: 4, charisma: 3 }, { requires: { stats: { charisma: 35 } } }),
    ...
    event('company_cutbacks', 'İşyerinde kemer sıkma', 'Şirket maliyetleri azaltıyor. Ekipte herkes gelecek yılın nasıl olacağını konuşuyor.', '🏢', 20, 64, [
      ...
      choice('Dışarıdaki mesleki çevreni genişlet', 'Belirsizliğe karşı yeni bağlantılar kurdun.', { charisma: 4, meet: 'colleague', stress: -2 }),
    ...
    event('partner_plans', 'Aynı hafta sonu, iki plan', 'Partnerin birlikte zaman bekliyor; sen ise kendi planına söz verdin. İkiniz de anlaşılmak istiyorsunuz.', '💞', 18, 90, [
      choice('Birlikte yeni bir plan kur', 'Her ikinizin isteğine yer açan bir yol buldunuz.', { bond: 7, charisma: 2, happiness: 4 }),
    ...
    event('parents_repair', 'Ailenin evinde bir arıza', 'Ailenin evindeki eski tesisat masraf çıkardı. Onarım için destek istiyorlar.', '🔧', 18, 65, [
      ...
      choice('Uygun bir usta bulmalarına yardım et', 'Para veremesen de çözüm aramak işe yaradı.', { bond: 3, charisma: 2 })
    ], { npcRole: 'parent' }),
    ...
    event('neighbour_exchange', 'Mahallede takas günü', 'Komşular kullanmadıkları eşyaları getiriyor. Para harcamadan yeni bir şey öğrenme fırsatı var.', '♻️', 8, 95, [
      ...
      choice('Organizasyona yardım et', 'Komşularını daha yakından tanıdın.', { charisma: 3, happiness: 4, meet: 'friend' }),
    ...
    event('sleep_routine', 'Uykusuz sabahlar', 'Son zamanlarda geç yatıp erken kalkıyorsun. Gün içinde dikkatinin dağıldığını fark ettin.', '🌙', 14, 85, [
      ...
      choice('Dinlenme ihtiyacını yakınlarınla konuş', 'Beklentileri paylaşmak üzerindeki baskıyı azalttı.', { charisma: 2, stress: -6, bond: 3 })
    ]),
    ...
    event('midlife_choice', 'Yıllar nereye gidiyor?', 'Eski bir fotoğrafa bakarken kurduğun hayatı düşünüyorsun. Bazı hayaller değişmiş, bazıları hâlâ orada.', '🪞', 38, 55, [
      ...
      choice('Büyük ve pahalı bir değişiklik yap', 'Yeni deneyim heyecan verdi; bütçende ciddi bir iz bıraktı.', { happiness: 12, charisma: 2, stress: -5 }, { cost: 25000 })
    ], { once: true }),
    event('old_friend', 'Yıllar sonra aynı ses', 'Uzun zamandır görmediğin bir tanıdık seni aradı. İkiniz de başka hayatlar yaşamışsınız.', '📻', 35, 90, [
      choice('Bir yürüyüş için buluş', 'Eski hikâyeler yeni bir dostluğa dönüştü.', { happiness: 6, charisma: 3, meet: 'friend' }),
    ...
    event('community_garden', 'Mahalle bostanında bir sıra', 'Mahalle bostanında gönüllüler yeni bir bölüm açıyor. Deneyim gerekmiyor, emek gerekiyor.', '🌱', 30, 95, [
      ...
      choice('Kendi setinle fide yetiştir', 'Fidelerin bostana taşındı. Bir şeyin büyümesini görmek sevindirdi.', { happiness: 7, knowledge: 4, charisma: 2 }, { requires: { items: ['garden_kit'] } }),
    ...
    event('retirement_plans', 'Çalışmanın ötesindeki günler', 'İleri yıllarda zamanını ve bütçeni nasıl kullanacağını düşünmeye başladın.', '🗓️', 55, 67, [
      ...
      choice('Yeni bir sosyal çevre kur', 'İş dışındaki hayatına daha çok yer açtın.', { happiness: 5, charisma: 3, meet: 'friend' }),
    ...
    event('quiet_house', 'Ev biraz daha sessiz', 'Günlük koşuşturma azaldı. Sessizlik bazen huzur, bazen de yalnızlık gibi geliyor.', '🪟', 58, 100, [
      choice('Düzenli bir topluluğa katıl', 'Takviminde beklediğin bir gün ve yeni bir dost oldu.', { happiness: 7, charisma: 3, meet: 'friend', stress: -4 }),
    ...
    event('legacy_request', 'Bir gencin sorusu', 'Senden yaşça küçük biri, yaptığı bir seçim için tavsiye istiyor. Kendi gençliğini hatırladın.', '🕯️', 55, 105, [
      choice('Başarıların kadar hatalarını da anlat', 'Dürüst hikâyen karşılık buldu. Deneyimin işe yaradı.', { happiness: 7, charisma: 3, knowledge: 2 }),
      choice('Kararı onun adına vermeden dinle', 'Kendi cevabını bulmasına alan açtın.', { happiness: 5, charisma: 4, stress: -3 }),
